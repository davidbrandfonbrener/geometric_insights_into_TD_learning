...
def parse_args():
    ...
    parser.add_argument('--delta', default=0.4, type=float)
    ...
    parser.add_argument('--save_path', default="../outputs/data/nn_init/")
...
def run_experiment(args):
    ...
    return tabular_Vs, mlp_Vs, condition_numbers, bound, smoothness
...
def main():
    ...
    for seed in seeds:
        ...
        args.seed = seed
        tabular_Vs, mlp_Vs, condition_numbers, bound, smoothness = run_experiment(args)
        t.append(tabular_Vs)
        m.append(mlp_Vs)
    ...
    np.savez(args.save_path  + "k_" + str(args.k) + "_n_" + str(args.n) + "_mlp_depth_" + str(args.depth) 
            ...
            tabular_Vs = t, mlp_Vs = m, condition_numbers = c, bound = b, seeds= np.array(seeds), smoothness= s)
